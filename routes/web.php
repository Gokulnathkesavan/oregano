...
Route::group(['middleware' => ['auth'], 'as' => 'admin.', 'prefix' => 'admin'], function () {
    ...
    Route::get('/entries_table', [EntryController::class, 'index'])->name('entries.index');

    Route::get('/entries/export-csv', [EntryController::class, 'exportCsv'])->name('entries.export.csv');
    ...
    //blog
    Route::view('details-blog', 'blog.blog')->name('blog');
    ...
    Route::view('outlet', 'outlet.outlet')->name('outlet');
    ...
    Route::post('outlets/{outlet}/generate-qr', [OutletController::class, 'generateQr'])->name('admin.outlets.generateQr');

 
    // Simple test route (add this at the top of your routes file)
Route::get('/test-route', function() {
    return response()->json([
        'status' => 'success',
        'message' => 'Test route is working!',
        'route_names' => [
            'admin_entries' => route('admin.entries.index'),
            'entry_form' => route('entry.form', ['slug' => 'test-slug']),
            'store_entry' => route('entries.store')
        ]
    ]);
})->name('test.route');

Route::view( 'table_test','entry.table.entries');

});


Route::get('/entry/{slug}', [EntryController::class, 'showForm'])->name('entry.form');
Route::post('/entries', [EntryController::class, 'store'])->name('entries.store');

// Route::view('entry', 'entry.reg_form')->name('entry');
